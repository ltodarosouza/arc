...
export default function ExplorePage() {
  ...
  const subjectDetails = useMemo(() => selectedSubjects.map((subject) => {
    const questions = seedQuestions.filter((question) => question.subjectId === subject.id);
    const topics = seedTaxonomyNodes.filter((node) => node.subjectId === subject.id && node.kind === 'topic').map((topic) => ({
      ...topic,
      count: questions.filter((question) => {
        const branch = getTaxonomyBranch(topic.id, seedTaxonomyNodes);
  ...
  }), [attemptedQuestionIds, selectedSubjects]);
