...
if (![1, 2].includes(batchNumber))
  throw new Error('Use --batch=01 ou --batch=02.');
const padded = String(batchNumber).padStart(2, '0');
const outputPath = resolve(
  root,
  `supabase/migrations/2026090918${batchNumber + 20}000_diversify_calculus_1_batch_${padded}.sql`,
);
...
const validate = (text, number) => {
  const normalized = normalize(text);
    ...
    `LaTex inválido na questão ${number}.`,
  );
...
const batch = createDiverseCalculusOneBatch(padded);
  ...
  new Set(batch.map((q) => q.statement)).size === 125,
  'Há enunciados repetidos.',
);
const lines = [
  `-- Banco Cálculo I renovado — lote ${padded}: 125 questões autorais revisadas.`,
  '-- Execute os lotes 01 e 02 antes da limpeza final.',
  'begin;',
  '',
];
if (batchNumber === 1)
  lines.push(
    `update public.questions set publication_status = 'draft' where subject_id = ${sql(subjectId)} and publication_status = 'published' and coalesce(source_id::text, '') not in (${sql('10000000-0000-4000-8000-000000000242')}, ${sql('10000000-0000-4000-8000-000000000243')});`,
    '',
  );
lines.push(
  'insert into public.question_sources (id, kind, label, licence_note, rights_holder, rights_status, verified_by, verified_at)',
  `values (${sql(sourceId)}, 'original', ${sql(`Arc original Cálculo I — banco renovado, lote ${padded}`)}, ${sql('Questões autorais da Arc, revisadas para diversidade, clareza pedagógica e consistência matemática.')}, 'Arc', 'approved', 'Equipe editorial Arc', '2026-09-09T00:00:00Z')`,
...
for (const [index, q] of batch.entries()) {
  ...
  const optionIds = q.options.map((_, i) => id(5, number * 10 + i + 1));
  ensure(topics[q.topic], `Tópico inválido: ${q.topic}`);
  ensure(
    q.options.length === 4 && new Set(q.options).size === 4,
    `Alternativas inválidas ${number}`,
  );
  ensure(
    q.hints.length >= 3 && q.steps.length >= 5 && q.explanation.length >= 240,
  ...
  lines.push(
    ...
    `insert into public.question_answer_keys (question_id, correct_option_id) values (${sql(questionId)}, ${sql(optionIds[q.correct])}) on conflict (question_id) do update set correct_option_id=excluded.correct_option_id;`,
    `insert into public.question_taxonomy_tags (question_id, taxonomy_node_id, is_primary) values (${sql(questionId)}, ${sql(topics[q.topic])}, true) on conflict (question_id, taxonomy_node_id) do update set is_primary=excluded.is_primary;`,
    `insert into public.question_hints (id, question_id, content_markdown, sort_order) values ${q.hints.map((hint, i) => `(${sql(id(6, number * 10 + i + 1))}, ${sql(questionId)}, ${sql(hint)}, ${i + 1})`).join(', ')} on conflict (id) do update set question_id=excluded.question_id, content_markdown=excluded.content_markdown, sort_order=excluded.sort_order;`,
    `insert into public.question_solutions (question_id, final_answer_markdown, explanation_markdown) values (${sql(questionId)}, ${sql(q.finalAnswer)}, ${sql(q.explanation)}) on conflict (question_id) do update set final_answer_markdown=excluded.final_answer_markdown, explanation_markdown=excluded.explanation_markdown;`,
...
if (process.argv.includes('--check')) {
    ...
    `Migration ${padded} desatualizada.`,
  );
  console.log(`Cálculo I v2 lote ${padded} validado.`);
} else {
