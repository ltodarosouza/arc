...
if (!/^(0[1-8])$/.test(batch ?? ''))
  throw new Error('Use --batch=01 até --batch=08.');
const root = resolve(import.meta.dirname, '..');
const questions = (
  await import(`../content/vector-calculus/authorial-batch-${batch}.mjs`)
).default;
...
if (questions.length !== 20)
  throw new Error(`Lote ${batch} precisa ter 20 questões.`);
if (new Set(questions.map((q) => q.statement)).size !== 20)
  throw new Error(`Lote ${batch} tem enunciados repetidos.`);
for (const [index, q] of questions.entries()) {
  if (!topics[q.topic] || !['easy', 'medium', 'hard'].includes(q.difficulty))
    throw new Error(`Taxonomia inválida em ${index + 1}.`);
  if (
    q.options.length !== 4 ||
    ...
    throw new Error(`Alternativas inválidas em ${index + 1}.`);
  if (q.hints.length < 3 || q.steps.length < 4 || q.explanation.length < 60)
    throw new Error(`Conteúdo insuficiente em ${index + 1}.`);
  [
    q.statement,
    ...q.options,
    ...q.hints,
...
questions.forEach((q, index) => {
  ...
  const questionId = id(4, number),
    optionIds = q.options.map((_, i) => id(5, number * 10 + i + 1));
...
const path = resolve(
  root,
  `supabase/migrations/2026090922${String(10 + batchIndex).padStart(2, '0')}00_seed_authorial_vector_calculus_batch_${batch}.sql`,
);
if (process.argv.includes('--check')) {
  if (readFileSync(path, 'utf8') !== out)
    throw new Error(`Migration ${batch} desatualizada.`);
