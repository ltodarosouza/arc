...
export function PracticeSurface() {
  ...
  return <ArcCard className="mt-8 overflow-hidden"><div aria-atomic="true" aria-live="polite" className="sr-only">{resolved ? outcome === 'correct' ? 'Resposta correta. Gabarito comentado disponível.' : 'Resposta incorreta. Gabarito comentado disponível.' : ''}</div><div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-4 sm:px-8"><div className="flex min-w-0 items-center gap-2 text-xs text-[var(--arc-text-muted)]"><Compass className="size-3.5 shrink-0" /><span className="truncate">{subject?.name}</span>{topic && <><ChevronRight className="size-3 shrink-0" /><span className="truncate">{topic.name}</span></>}</div>{resolved ? <AttemptStatusBadge status={outcome} /> : <span className="rounded-full bg-[var(--arc-surface-subtle)] px-3 py-1 text-xs font-medium text-[#52616c]">Questão {String(questionIndex + 1).padStart(2, '0')}</span>}</div>
    <div className="p-5 sm:p-10"><div className="max-w-2xl text-xl font-medium leading-relaxed tracking-[-0.035em] sm:text-2xl"><MathContent value={question.statement.value} /></div>
      ...
      {visibleHints.length > 0 && <section aria-label="Dicas" id="question-hints" className="mt-6 max-w-2xl rounded-2xl border border-[#d5e1dc] bg-[#f3f7f4] p-4 text-sm leading-6 text-[#456252]"><p className="font-medium">Dica{visibleHints.length > 1 ? 's' : ''}</p><ol className="mt-2 grid gap-2">{visibleHints.map((hint, index) => <li key={hint.id}><span className="mr-2 font-medium text-[#668172]">{index + 1}.</span><MathContent value={hint.content.value} /></li>)}</ol></section>}
      {resolved && <><div className={`mt-6 max-w-2xl rounded-2xl p-4 text-sm leading-6 ${outcome === 'correct' ? 'bg-[var(--arc-success-bg)] text-[var(--arc-success-text)]' : 'bg-[var(--arc-error-bg)] text-[var(--arc-error-text)]'}`}><p className="font-medium">{outcome === 'correct' ? 'Você acertou.' : 'Você errou.'}</p></div>{solution && <section aria-label="Gabarito comentado" className="mt-6 max-w-2xl border-t border-[var(--border)] pt-6"><div className="flex items-baseline justify-between gap-4"><h2 className="text-lg font-medium tracking-[-0.03em]">Gabarito comentado</h2><span className="text-xs text-[var(--arc-text-muted)]">Resposta</span></div><div className="mt-4 rounded-2xl bg-[var(--arc-surface-subtle)] p-4 text-[15px] leading-6"><MathContent value={solution.finalAnswer} /></div>{solution.explanation && <div className="mt-4 text-sm leading-6 text-[var(--arc-text-muted)]"><MathContent value={solution.explanation} /></div>}{solution.steps.length > 0 && <ol className="mt-5 grid gap-3">{solution.steps.map((step, index) => <li className="flex gap-3 text-sm leading-6" key={step.id}><span className="grid size-6 shrink-0 place-items-center rounded-full bg-[var(--arc-accent)] text-xs font-medium text-[#405b6d]">{index + 1}</span><div>{step.title && <p className="font-medium">{step.title}</p>}<MathContent value={step.content} /></div></li>)}</ol>}</section>}</>}
      {submissionError && <p className="mt-4 text-sm text-[var(--arc-error-text)]">{submissionError}</p>}
      <div className="mt-10 flex flex-wrap items-center justify-between gap-4">{hasMoreHints && !resolved ? <button aria-expanded={visibleHintCount > 0} aria-controls="question-hints" className="inline-flex items-center gap-2 text-sm text-[var(--arc-text-muted)] transition-colors hover:text-[var(--foreground)]" onClick={() => setVisibleHintCount((count) => Math.min(count + 1, question.hints.length))}><CircleHelp className="size-4" /> {visibleHintCount ? 'Ver próxima dica' : 'Preciso de uma dica'}</button> : <span />}<div className="flex items-center gap-3">{resolved && nextQuestion && <button className="inline-flex items-center gap-1 text-sm font-medium text-[#46657a] hover:underline" onClick={() => goToQuestion(nextQuestion.id, nextQuestion.subjectId)}>Próxima <MoveRight className="size-4" /></button>}<ArcButton disabled={!selectedOptionId || resolved} onClick={() => void submitAnswer()}><Check className="size-4" /> Responder</ArcButton></div></div>
    </div>
  </ArcCard>;
}
