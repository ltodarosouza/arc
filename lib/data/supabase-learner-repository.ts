...
function toAttempt(row: DatabaseAttempt): QuestionAttempt {
  return {
    ...
    answer: row.selected_option_id
      ...
      : { kind: 'revealed_answer', selfAssessment: row.outcome === 'correct' ? 'correct' : row.outcome === 'incorrect' ? 'incorrect' : 'not_assessed' },
    outcome: row.outcome,
...
export async function loadSupabaseLearnerState(): Promise<LearnerState> {
  ...
  const [subjectsResult, attemptsResult, redoResult] = await Promise.all([
    ...
    supabase.from('question_attempts').select('id, question_id, selected_option_id, outcome, grading_method, created_at').eq('user_id', userId).order('created_at'),
    supabase.from('redo_questions').select('question_id').eq('user_id', userId),
  ]);
  const error = [subjectsResult, attemptsResult, redoResult].find((result) => result.error)?.error;
  if (error) throw error;
  return {
    version: 1,
    selectedSubjectIds: (subjectsResult.data ?? []).map((row) => row.subject_id),
...
export async function saveSupabaseSelectedSubjects(subjectIds: string[]) {
  const supabase = getSupabaseClient();
  const userId = await currentUserId();
  const legacySlugs = subjectIds.map((id) => subjectOptions.find((subject) => subject.id === id)?.slug).filter((slug): slug is string => Boolean(slug));
  ...
  const durableIdBySlug = new Map((durableSubjects ?? []).map((subject) => [subject.slug, subject.id]));
  const durableIds = subjectIds.map((id) => durableIdBySlug.get(subjectOptions.find((subject) => subject.id === id)?.slug ?? '') ?? id);
  const { data: existing, error: existingError } = await supabase.from('user_subjects').select('subject_id').eq('user_id', userId);
  ...
  if (toAdd.length) {
    const { error } = await supabase.from('user_subjects').insert(toAdd.map((subject_id) => ({ user_id: userId, subject_id })));
    if (error) throw error;
  }
  if (toRemove.length) {
    const { error } = await supabase.from('user_subjects').delete().eq('user_id', userId).in('subject_id', toRemove);
    if (error) throw error;
  }
}
...
export async function setSupabaseRedo(questionId: string, enabled: boolean) {
  ...
  const result = enabled
    ? await supabase.from('redo_questions').upsert({ user_id: userId, question_id: questionId }, { onConflict: 'user_id,question_id' })
    : await supabase.from('redo_questions').delete().eq('user_id', userId).eq('question_id', questionId);
  if (result.error) throw result.error;
}

/** Explicit offline shape for callers that need to recover from a remote failure. */
export function emptyRemoteLearnerState() { return createEmptyLearnerState(); }
