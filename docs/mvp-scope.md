...
5. They answer a multiple-choice question.
...
- Email-and-password account creation and sign-in, with durable selected
  subjects, attempts, and redo state in Supabase.
...
- The local seeded catalogue remains a development fallback; production data is
  read from the reusable database/API boundary.
...
- Which account providers beyond email/password are worth supporting.
